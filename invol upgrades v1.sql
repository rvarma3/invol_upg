...
with invol_booking as
				...
				to_date(a18.CALENDAR_DATE) =  '2022-06-18' and
...
flown_class	as			(
				...
				from
						...
						from	"EDWH_PROD"."SILVER"."FLOWN"	a11
							...
							join	"EDWH_PROD"."SILVER"."PNR_DETAILS"	a13
							  on 	(a11.PNR_SK = a13.PNR_SK and a11.ticket_sk = a13.ticket_sk and a11.OPERATIONAL_FLT_SK = a13.OPERATIONAL_FLIGHT_NO_SK )
							...
							join	"EDWH_PROD"."SILVER"."AGENT"	a23
							  on 	(a11.AGENT_SK = a23.AGENT_SK)
							join	"EDWH_PROD"."SILVER"."GEO_LOCATION"	a24
							  on 	(a23.GEO_LOCATION_SK = a24.LOCATION_SK)
						...
						to_date(a14.CALENDAR_DATE) = '2022-06-18' and
...
emd_table as
		...
		count(distinct TKT_OPERATIONAL_FLIGHT_NO_SK) as flight_count
			...
			join	"EDWH_PROD"."SILVER"."PNR_DETAILS"	a16
			  on 	(a11.PNR_SK = a16.PNR_SK and a11.tkt_sk = a16.TICKET_SK and a11.TKT_OPERATIONAL_FLIGHT_NO_SK = a16.OPERATIONAL_FLIGHT_NO_SK )
...
miles_upgade as 
				( -- all miles transactions for miles
					...
					a11.BOOKABLE_ACTIVITY_DIFFERENTIATOR_CODE like '%OWU%' and
				 	...
				 	to_date(a12.MEMBER_ACTIVITY_DATE) = '2022-06-18' and
...
where
ss1.departure_date =  '2022-06-18' and
ss1.FLIGHT_DESC = '0124' and
1=1
group by
ss1.FLIGHT_DESC,
ss1.BOOKING_REFERENCE,
